...
// });
  ...
  const columns = [
    ...
    //REMAINING
    {
      ...
      dataIndex: 'TotRem',
      align: "right",
      // render: (value5) => formatDec(value5),
      render: (value, record) =>
      record.IsTotal === 1 ? (
        <strong style={{ color: "#4f46e5", fontWeight: 600 }}>
          {formatDec(value)}
    ...
    if (!!(record.IsTotal === 1) || !!(record.IsGTotal === 1) ) 
      {return null;}
    else {
        if ((record.TotRem === 0)){
...
// }, [stfeesmtrx]);
  return (
      ...
      <h3 style={{textAlign:"center"}}><u><b>{scnm}</b></u></h3>
      <h3 style={{textAlign:"center"}}><u><b>Student Fees Report - {import.meta.env.VITE_CUR_YEAR_NAME}</b></u></h3>
      <br></br>
      <div className="yrr">
      ...
      <div>{loading ? (<p></p>) : (stfeesmtrx.length > 0 ? ( <Button className="prntTb" onClick={tbPrnt}>Print / Save As PDF</Button>): (<></>)  )}</div>
{/* <div>
  {!loading && (
    <Button
      className="prntTb"
