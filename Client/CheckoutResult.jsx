...
const sendEmailToSchool = async () => {
  try {
    ...
    await axios.post("http://localhost:3001/api/send-receipt-email", payload);
...
const sendWhatsappToSchool = async () => {
  try {
    ...
    const payload = {
      ...
      receiptData: {
        parentEmail: details.customer_email,
        amount: formatDec(details.amount) / 100,
        fort_id: details.fort_id,
        merchant_reference: details.merchant_reference,
        response_message: details.response_message,
        status: details.status,
    ...
    //const res = await axios.post(`${REACT_APP_API_BASE || "http://localhost:3000"}/api/send-receipt-whatsapp`, payload);
    const { data: waData } = await axios.post(`${REACT_APP_API_BASE}/api/generate-whatsapp-link`, {...payload, publicUrl: pdfData.publicUrl});
    console.log("WhatsApp link:", waData.waLink);
