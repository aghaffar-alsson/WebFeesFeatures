...
function SignUp({userData}) {
  ...
  const createLogin = async (e) => {
    // e.preventDefault();
    // if already tapped once, never allow again
    if (btnSubmitLocked) return;
  ...
  // console.log("Temp Pswd Valid:", isTempPswdValid, "User Temp Pswd:", usrtmpPss, "System Temp Pswd:", systempPswd)
  // console.log("Temp Pswd Valid:", isTempPswdValid)
  ...
  return (
    <>
      <form className="allDivv">
        ...
        {/* Family Info */}
        {!isFormValid ? (<p></p>) : (<div ><strong className="fminfo">Family ID:{fmDtt.famid} - Family Name:{fmDtt.famnm} </strong></div>)}
        {/* Submit */}
        <div className="sbmtt">
          <button className={isFormValid && !btnSubmitLocked ? "enbtn" : "disbtn"} type="submit" tabIndex="9" id="btnSubmit" 
          disabled={!isFormValid || btnSubmitLocked} onClick={createLogin}>
          {btnSubmitLocked ? (loginCreated ? "Login Created" : "Request Submitted"): "Create Login"}{" "}
